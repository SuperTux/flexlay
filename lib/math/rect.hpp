...
public:
	...
	Rectf(const Pointf &p, const Sizef &size)
	{ left = p.x; top = p.y; right = left + size.width; bottom = top + size.height; }
	...
	//: Rect + Point operator.
	Rectf operator+(const Pointf &p) const
...
public:
	...
	//param x, y: Offsets applied negatively to the hotspot point
	//param angle: Angle to rotate in degrees.
	Rectf get_rot_bounds(const Pointf &hotspot, float angle) const;
	Rectf get_rot_bounds(Origin origin, float x, float y, float angle) const;
...
public:
	...
	{
		Pointf offset = calc_origin(origin, get_size());
		offset.x -= x;
		offset.y -= y;
