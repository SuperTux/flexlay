...
Rectf Rectf::get_rot_bounds(const Pointf &hotspot, float angle) const
{
	//Find the rotated positions of each corner
	Rectf retVal(*this);
	Pointf ul = Pointf(retVal.left, retVal.top).rotate(hotspot, angle);
	Pointf ur = Pointf(retVal.right, retVal.top).rotate(hotspot, angle);
	Pointf ll = Pointf(retVal.left, retVal.bottom).rotate(hotspot, angle);
	Pointf lr = Pointf(retVal.right, retVal.bottom).rotate(hotspot, angle);
...
Rectf Rectf::get_rot_bounds(Origin origin, float x, float y, float angle) const
{
	return get_rot_bounds(
		Pointf(left, top) + calc_origin(origin, get_size()) + Pointf(x, y),
		angle);
}
